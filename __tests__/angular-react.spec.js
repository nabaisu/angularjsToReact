...
import { ngRepeatFn } from "../angular/ng-stuff/ng-repeat";
import { inputNgRepeat, outputNgRepeat } from "./checks/ng-repeat";
import { ngRepeatExpression } from "../angular/expressions/ng-repeat";
...
describe("Angular to React", () => {
  describe("Angular Element Attributes", () => {
    ...
    it("ng-repeat", () => {
      const output = transform(inputNgRepeat, ngRepeatFn);
      expect(output.trim()).toEqual(outputNgRepeat.trim());
    }); //+ 318
  ...
  describe("expressions", () => {
    describe("ng-repeat", () => {
      it("item in items => {variable, expression}", () => {
        const input = "item in items";
        const expectedOutput = { variable: ["item"], expression: ["items"] };
        const output = ngRepeatExpression(j, input);
        expect(output).toEqual(expectedOutput);
      });
      it("(key, value) in myObj => {variable, expression}", () => {
        const input = "(key, value) in myObj";
      ...
      it('(name, age) in {"adam":10, "amalie":12} => {variable, expression}', () => {
        const input = "(name, age) in {'adam':10, 'amalie':12}";
        const expectedOutput = {
          variable: ["name", "age"],
          expression: { adam: 10, amalie: 12 },
        };
        const output = ngRepeatExpression(j, input);
        expect(output).toEqual(expectedOutput);
      });
